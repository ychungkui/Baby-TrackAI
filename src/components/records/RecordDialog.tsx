      ...
      if (editRecord) {
        ...
        form.reset({
          ...
          end_time: editRecord.end_time
            ? format(new Date(editRecord.end_time), "yyyy-MM-dd'T'HH:mm")
            : '',
          amount_ml: ((notes as FeedingNotes)?.amount_ml || (notes as WaterNotes)?.amount_ml)?.toString() || '',
          duration_minutes: ((notes as SleepNotes)?.duration_minutes ||
            (notes as NightWakeNotes)?.duration_minutes ||
            (notes as BathNotes)?.duration_minutes)?.toString() || '',
          amount_grams: (notes as SolidFoodNotes)?.amount_grams?.toString() || '',
        });

        if (editRecord.type === 'feeding') {
          setFeedingType((notes as FeedingNotes)?.feeding_type || 'bottle');
        }
        if (editRecord.type === 'diaper') {
          setDiaperType((notes as DiaperNotes)?.diaper_type || 'wet');
        }
        if (editRecord.type === 'potty') {
          setPottyType((notes as PottyNotes)?.diaper_type || 'wet');
        }
  ...
  const onSubmit = async (values: any) => {
    ...
    if (selectedTime > new Date()) {
      form.setError('start_time', {
        message: t('records.no_future_time')
      });
      return;
    }
    ...
    try {
      ...
      switch (recordType) {
        case 'feeding': {
          const fn: FeedingNotes = { feeding_type: feedingType };
          if (values.amount_ml) fn.amount_ml = parseInt(values.amount_ml, 10);
          notes = fn;
          break;
        }
        case 'diaper':
          notes = { diaper_type: diaperType } as DiaperNotes;
          break;
        case 'potty':
          notes = { diaper_type: pottyType } as PottyNotes;
          break;
        case 'sleep':
        case 'night_wake':
        case 'bath':
          if (values.duration_minutes) {
            notes = { duration_minutes: parseInt(values.duration_minutes, 10) };
          }
          break;
        case 'water':
          if (values.amount_ml) {
            notes = { amount_ml: parseInt(values.amount_ml, 10) } as WaterNotes;
          }
          break;
        case 'solid_food':
          if (values.amount_grams) {
            notes = { amount_grams: parseInt(values.amount_grams, 10) } as SolidFoodNotes;
          }
          break;
      }
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('records.time')}</FormLabel>
                    <FormControl>
                      <Input
                        type="datetime-local"
              ...
              {recordType === 'feeding' && (
                <>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('records.amount_ml')}</FormLabel>
                        <FormControl>
                          <Input
                            type="number"
                            className="h-12"
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {(isAdding || isUpdating)
                    ? t('records.saving')
                    : isEditing
                      ? t('records.confirm_edit')
                      : t('records.confirm_record')}
                </Button>
              </div>
